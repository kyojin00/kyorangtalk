...
interface Friend { id: string; requester_id: string; receiver_id: string; requester?: any; receiver?: any }
      ...
      <header className="sticky top-0 z-50" style={{ background: 'var(--surface)', borderBottom: '1px solid rgba(108,92,231,0.1)' }}>
        ...
        <div className="max-w-lg mx-auto px-4 flex">
          ...
          <button onClick={() => setTab('friends')} className="flex-1 py-2.5 text-sm font-medium transition-all relative" style={{ color: tab === 'friends' ? 'var(--purple)' : 'var(--muted)', borderBottom: tab === 'friends' ? '2px solid var(--purple)' : '2px solid transparent' }}>
            친구
            {pending.length > 0 && <span className="absolute top-2 right-8 w-4 h-4 rounded-full text-white text-xs flex items-center justify-center" style={{ background: '#E74C3C' }}>{pending.length}</span>}
          </button>
        </div>
      </header>
      ...
      <div className="max-w-lg mx-auto p-4">
          ...
          <div className="space-y-2">
            {rooms.length === 0 ? (
              ...
              rooms.map((room) => {
                ...
                const partnerFriend = friends.find(f => getFriendUserId(f) === partnerId)
                const partnerProfile = partnerFriend ? getFriendProfile(partnerFriend) : null
                return (
                  <button key={room.id} onClick={() => router.push(`/chat/${room.id}`)} className="w-full rounded-xl p-4 flex items-center gap-3 hover:opacity-80 transition text-left" style={{ background: 'var(--surface)', border: '1px solid rgba(108,92,231,0.1)' }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-sm" style={{ color: 'var(--text)' }}>{partnerProfile?.nickname || '알 수 없음'}</span>
        ...
        {tab === 'friends' && (
          <div className="space-y-4">
            ...
            {pending.length > 0 && (
              <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(108,92,231,0.1)' }}>
                <p className="text-xs font-medium px-4 py-2" style={{ background: 'var(--bg)', color: 'var(--muted)' }}>받은 친구 요청 {pending.length}개</p>
                {pending.map((req) => (
            ...
            {/* 친구 목록 */}
            <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(108,92,231,0.1)' }}>
              <p className="text-xs font-medium px-4 py-2" style={{ background: 'var(--bg)', color: 'var(--muted)' }}>친구 {friends.length}명</p>
              {friends.length === 0 ? (
